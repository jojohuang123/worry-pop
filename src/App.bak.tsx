/**
 * BACKUP OF App.tsx - Version: Satisfied Dark Glass
 */
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Send, RotateCcw, Sparkles, Zap, Heart, Volume2, VolumeX, MousePointer2, CloudRain, Flame } from 'lucide-react';
...
export default function App() {
  ...
  const analyzeWorry = useCallback(() => {
    ...
    // Deep Violet/Magenta Palette
    const factor = Math.min(resentmentCount / 6, 1);
    const r = Math.floor(90 + (220 - 90) * factor);
    const g = Math.floor(80 + (40 - 80) * factor);
    const b = Math.floor(230 + (120 - 230) * factor);
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden transition-all duration-150 ${isCriticalZone && hp > 0 ? 'screen-shake-active bg-red-950/30' : ''}`}>
      ...
      <button 
        ...
        className="absolute top-8 right-8 z-50 p-3 glass-panel rounded-full text-white/30 hover:text-white hover:scale-110 transition-all"
      >
      ...
      <AnimatePresence mode="wait">
        {!analysis && !isPopped && (
          <motion.div
            ...
            className="z-10 w-full max-w-[546px] glass-panel p-8 md:p-10 rounded-[2.5rem] shadow-2xl border-white/5 bg-[#030202] flex flex-col items-center"
          >
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2.5 bg-violet-500/20 rounded-2xl">
                <CloudRain className="text-violet-400" size={24} />
              </div>
              <h1 className="text-3xl font-black tracking-tight text-glow">烦恼粉碎机</h1>
            </div>
            <p className="text-gray-500 mb-6 text-xs font-semibold tracking-wide uppercase">释放压力，手速决定消散速度。</p>
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-6 w-full max-w-[430px]">
              {suggestions.map((s, i) => (
                <button
                  key={i}
                  onClick={() => setWorry(s)}
                  className="text-[13px] px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-gray-400 hover:text-gray-200 transition-all active:scale-95 font-bold font-[Arial]"
                >
            ...
            <textarea
              ...
              placeholder="输入你的烦恼... 关键词越多，气球越坚韧。"
              className="w-full h-32 bg-white/[0.01] border border-white/10 rounded-2xl p-6 text-white placeholder:text-gray-800 focus:outline-none focus:ring-2 focus:ring-violet-500/20 transition-all resize-none mb-6 text-lg leading-relaxed"
            />

            <button
              onClick={analyzeWorry}
              disabled={!worry.trim()}
              className="w-full max-w-[431px] h-[58px] bg-gradient-to-br from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 disabled:from-gray-800 disabled:to-gray-900 disabled:text-gray-600 rounded-2xl font-black text-lg transition-all flex items-center justify-center gap-4 group shadow-2xl shadow-violet-900/30 active:scale-[0.97] mx-auto"
            >
        ...
        {analysis && !isPopped && (
          ...
          >
            {/* Combo Display */}
            <div className="h-12 md:h-20 mb-2 md:mb-4 flex items-center justify-center">
              <AnimatePresence>
                {combo > 3 && (
                  <motion.div
                    ...
                    className="text-4xl md:text-7xl font-black italic bg-gradient-to-r from-yellow-300 via-orange-500 to-red-500 bg-clip-text text-transparent drop-shadow-2xl"
                  >
                    {combo}
                  </motion.div>
                )}
              </AnimatePresence>
              {combo > 3 && <span className="ml-2 text-xl md:text-4xl font-black italic text-orange-500">COMBO!</span>}
            </div>
            ...
            <motion.div
              ...
              animate={{
                ...
                scale: (window.innerWidth < 768 ? 0.8 : 1) + (maxHp - hp) * 0.004 + (combo * 0.008),
            ...
            >
              ...
              >
                <span className="text-white font-black text-2xl break-words drop-shadow-[0_4px_12px_rgba(0,0,0,0.7)] leading-tight">
                  {worry}
                </span>
              </div>
              {/* String */}
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 w-full max-w-[85%] md:max-w-sm px-4 z-50">
              <div className="flex flex-col items-center gap-1">
                <div className={`flex items-center gap-2 font-black tracking-[0.2em] text-xs md:text-base uppercase transition-colors duration-300 ${isCriticalZone ? 'text-red-500 animate-pulse' : 'text-violet-400'}`}>
                  {isCriticalZone ? <Flame size={16} /> : <MousePointer2 size={16} className="animate-bounce" />} 
                  {isCriticalZone ? '即将引爆！！！' : '疯狂连点'}
                </div>
                <div className="text-[9px] text-gray-700 font-black tracking-[0.2em]">停下点击气球会慢慢恢复</div>
              </div>
              
              {/* HP Bar */}
              <div className="w-full space-y-2">
                <div className="w-full h-3 md:h-5 bg-white/5 rounded-full overflow-hidden border border-white/10 p-1 shadow-2xl">
                  <motion.div 
                    className={`h-full rounded-full transition-colors duration-300 ${isCriticalZone ? 'bg-red-500 shadow-[0_0_20px_rgba(239,68,68,0.8)]' : 'bg-gradient-to-r from-violet-600 via-fuchsia-600 to-pink-600'}`}
                    initial={{ width: '100%' }}
                    animate={{ width: `${hpPercent}%` }}
                  />
                </div>
                <div className="flex justify-between w-full text-[9px] md:text-xs uppercase tracking-[0.2em] font-black">
                  <span className="text-gray-600">怨念残余</span>
                  <span className={isCriticalZone ? 'text-red-500 scale-110 transition-transform' : 'text-violet-400'}>
                    {Math.ceil(hpPercent)}%
                  </span>
                </div>
              </div>
        ...
        {isPopped && (
          ...
          >
            ...
            <div className="glass-panel p-8 rounded-[2.5rem] mb-8 min-h-[140px] flex items-center justify-center italic text-xl leading-relaxed text-violet-100 border-violet-500/20 shadow-2xl relative overflow-hidden">
            ...
            <button
              onClick={reset}
              className="px-12 py-4 bg-gradient-to-br from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 rounded-[2rem] transition-all flex items-center gap-4 mx-auto font-black text-lg shadow-2xl shadow-violet-900/50 active:scale-95"
            >
